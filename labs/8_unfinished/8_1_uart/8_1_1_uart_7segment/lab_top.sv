...
(
    ...
    localparam over_smpl    = 0;        // 0: x16, 1: x8
    localparam stop_bits    = 0;        // 0: 1 stop bit, 1: 2 stop bits
    ...
    always_ff @(posedge clk or posedge rst) begin
        ...
        else if (rx_vld & rx_rdy) begin
            for (int i = w_digit - 1; i > 0; i--)
                rx_buf[i] <= rx_buf[i - 1];
            rx_buf[0] <= rx_data;
        end
    end
